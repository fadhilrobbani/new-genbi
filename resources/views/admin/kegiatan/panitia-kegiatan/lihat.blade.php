                    ...
                    <div class="col-12">
                        <div class="card">
                            <div class="card-header">
                                <h3 class="card-title">Panitia Kegiatan</h3>
                                <a href="/panitia-kegiatan/create" class="btn btn-primary btn-sm float-right"><i
                                        class="fas fa-plus mr-1"></i> Tambah Panitia Kegiatan</a>
                            </div>
                            <!-- /.card-header -->
                            <div class="card-body">
                                <table id="example2" class="table table-bordered table-hover lign-items-center mb-0">
                                    ...
                                    <tbody>
                                        <tr>
                                            @foreach ($panitia_kegiatans as $key => $panitia_kegiatan)
                                                {{-- {{ dd($panitia_kegiatan->user_id) }} --}}
                                                <td class="align-middle text-center">
                                                    {{ $panitia_kegiatans->firstItem() + $key }}</td>
                                                <td class="align-middle text-center">
                                                    {{ $panitia_kegiatan->user->nama }}</td>

                                                <td class="align-middle text-center">
                                                    {{ $panitia_kegiatan->jabatanKegiatan->jabatan }}
                                                </td>

                                                <td class="text-sm-center">
                                                    <form
                                                        action="{{ route('panitia-kegiatan.setuju', $panitia_kegiatan->user_id) }}"
                                                        method="post" class="d-inline">
                                                        @method('POST')
                                                        @csrf
                                                        <button type="submit" class="btn btn-primary btn-sm border-0"
                                                            onclick="return confirm('Anda yakin akan menyetujui peserta ini?')">
                                                            <i class="bi bi-ban"></i>
                                                            Setuju
                                                        </button>
                                                    </form>
                                                    <form
                                                        action="{{ route('panitia-kegiatan.tolak', $panitia_kegiatan->user_id) }}"
                                                        method="post" class="d-inline">
                                                        @method('POST')
                                                        @csrf
                                                        <button type="submit" class="btn btn-danger btn-sm border-0"
                                                            onclick="return confirm('Anda yakin akan menolak peserta ini?')">
                                                            <i class="bi bi-ban"></i>
                                                            Tolak
                                                        </button>
                                                    </form>
                                ...
                                <div class="row">
                                    <div class="col-12">
                                        <div class="mt-3 float-right">{{ $panitia_kegiatans->links() }}</div>
                                    </div>
                                </div>
                            </div>
                            <!-- /.card-body -->
