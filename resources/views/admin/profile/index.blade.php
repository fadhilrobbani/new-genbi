              ...
              <div class="card-body box-profile">
                <div class="text-center">
                  <img class="profile-user-img img-fluid img-circle"
                       src="{{ asset('storage/images/'. auth()->user()->foto) }}"
                       alt="User profile picture">
                </div>

                <h3 class="profile-username text-center" >{{ auth()->user()->nama }}</h3>

                <p class="text-muted text-center">{{ auth()->user()->komisariat->komisariat }}</p>

                <ul class="list-group list-group-unbordered mb-3">
                  <li class="list-group-item">
                    <b>Poin Keaktifan</b> <a class="float-right">{{ auth()->user()->poin }}</a>
                  </li>
                  <li class="list-group-item">
                    <b>Jabatan</b> <a class="float-right">{{ auth()->user()->jabatan->jabatan }}</a>
                  </li>
                  <li class="list-group-item">
                ...
                <form method="POST" action="{{ route('profile.updatefoto',auth()->user()->id) }}" enctype="multipart/form-data">
                  @csrf
              ...
              <div class="card-body">
                <div class="tab-content">
                  ...
                  <div class="tab-pane" id="settings">
                    <form action="profile/edit/{{ auth()->user()->id }}" class="form-horizontal" method="post" enctype="multipart/form-data">
                      @csrf
                      @method('put')
                        ...
                        <div class="col-sm-10">
                          <select class="custom-select rounded-0" name="komisariat_id" id="komisariat">
                            @foreach($komisariat as $komisariat)
                                <option value="{{ $komisariat->id }}" {{ auth()->user()->komisariat->id == $komisariat->id ? 'selected' : '' }}>
                        ...
                        <div class="col-sm-10">
                          <select class="custom-select rounded-0" name="jabatan_id" id="jabatan">
                            @foreach($jabatan as $jabatan)
                                <option value="{{ $jabatan->id }}" {{ auth()->user()->jabatan->id == $jabatan->id ? 'selected' : '' }}>
                                    {{ $jabatan->jabatan }}
                                </option>
                            @endforeach
                        </select>
                      ...
                      <div class="form-group row">
                        ...
                        <div class="col-sm-10">
                          <input type="number" class="form-control" id="angkatan" name="angkatan" value="{{ auth()->user()->angkatan }}">
                        </div>
                      </div>
                      <div class="form-group row">
                        <label for="poin" class="col-sm-2 col-form-label">Poin Keaktifan</label>
                        <div class="col-sm-10">
                          <input type="number" class="form-control" id="poin" name="poin" value="{{ auth()->user()->poin }}">
                        </div>
                      </div>
                      <div class="form-group row">
